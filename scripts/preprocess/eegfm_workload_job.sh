#!/usr/bin/env bash
# SLURM job body for EEG-FM-Bench Workload preprocessing.
...
"$PYTHON" scripts/preprocess/download_workload_to_eegfm.py

"$PYTHON" external/EEG-FM-Bench/preproc.py \
  conf_file=configs/eegfm_bench/workload_preproc.yaml
